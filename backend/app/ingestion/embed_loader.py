...
async def populate_statute_embeddings(session: AsyncSession, batch_size: int = 32) -> Dict[str, Any]:
    """
    ...
    stmt = (
        select(StatuteChunk.id, StatuteChunk.chunk_id, StatuteChunk.text, StatuteChunk.needs_review)
    ...
    logger.info(f"Populating embeddings for {total_chunks} statute chunks in batches of {batch_size}...")
    ...
    for i in range(0, total_chunks, batch_size):
        ...
        print(f"  [Embedding Progress] {processed_so_far}/{total_chunks} chunks embedded ({rate:.1f} chunks/sec)...")
