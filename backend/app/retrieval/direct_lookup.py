...
def detect_act_and_section_intent(query: str) -> Optional[Dict[str, str]]:
    ...
    return {"section": raw_sec, "base_section": base_sec, "act": act}
...
async def fetch_section_directly(
    session: AsyncSession, section_number: str
) -> List[Dict[str, Any]]:
    """
    ...
    """
    logger.info(
        "Performing deterministic direct lookup for Section %s (BNSS)...",
        section_number,
    )
    stmt = (
        select(StatuteChunk)
    ...
    if not chunks:
        logger.warning(
            "Direct BNSS section lookup found 0 chunks for Section %s.", section_number
        )
        return []
    ...
    logger.info(
        "Direct lookup returned %d chunks for Section %s.", len(results), section_number
    )
    return results


async def fetch_bns_offence_directly(
    session: AsyncSession, bns_section: str
) -> List[Dict[str, Any]]:
    """
    ...
    """
    logger.info(
        "Performing deterministic direct lookup for BNS Section %s (First Schedule)...",
        bns_section,
    )
    ...
    if not rows:
        logger.warning(
            "Direct BNS offence lookup found 0 rows for Section %s.", bns_section
        )
        return []
    ...
    logger.info(
        "Direct BNS offence lookup returned %d rows for Section %s.",
        len(results),
        bns_section,
    )
    return results
