...
if __name__ == "__main__":
    ...
    run_step(
        "[2/4] Ingesting BNSS Bare Act (531 sections)...",
        f'"{python_bin}" -m scripts.ingest_bns --with-embeddings',
    )
    run_step(
        "[3/4] Ingesting BNS First Schedule (474 rows)...",
        f'"{python_bin}" -m scripts.ingest_schedule --with-embeddings',
    )
    run_step(
        "[4/4] Extracting 58 Statutory Vector Forms...",
        f'"{python_bin}" -m scripts.extract_forms',
    )
