...
from app.retrieval.embeddings import embed_passages, check_token_length, get_embedding_model
...
async def populate_schedule_embeddings(batch_size: int = 32):
    ...
    async with AsyncSessionLocal() as session:
        result = await session.execute(
            select(OffenceClassification.id, OffenceClassification.bns_section, OffenceClassification.offence_description)
            .order_by(OffenceClassification.page_number, OffenceClassification.id)
        )
        records = result.all()

        total = len(records)
        logger.info("Populating embeddings for %d First Schedule records in batches of %d...", total, batch_size)
        ...
        for i in range(0, total, batch_size):
            ...
            print(f"  [Embedding Progress] {min(i + batch_size, total)}/{total} records embedded ({rate:.1f} rows/sec)...")
...
async def run_schedule_ingestion(with_embeddings: bool = False):
    ...
    async with AsyncSessionLocal() as session:
        total_loaded, needs_review_count = await load_offence_classification_to_db(session)

    # 3. Query Database Statistics & Summarize
    print("\n[3/4] Querying Database Summary Metrics & Classification Statistics...")
    async with AsyncSessionLocal() as session:
        result_total = await session.execute(select(func.count(OffenceClassification.id)))
        total_count = result_total.scalar()

        result_review = await session.execute(
            select(func.count(OffenceClassification.id)).where(OffenceClassification.needs_review == True)
        )
        review_count = result_review.scalar()

        result_cog = await session.execute(
            select(func.count(OffenceClassification.id)).where(OffenceClassification.cognizable.ilike("%Cognizable%"))
        )
        cognizable_count = result_cog.scalar()

        result_non_cog = await session.execute(
            select(func.count(OffenceClassification.id)).where(OffenceClassification.cognizable.ilike("%Non-cognizable%"))
        )
        non_cognizable_count = result_non_cog.scalar()

        result_bail = await session.execute(
            select(func.count(OffenceClassification.id)).where(OffenceClassification.bailable.ilike("%Bailable%"))
        )
    ...
    print(f" Clean Tail Extractions (needs_review=False):   {total_count - review_count}")
    ...
    for s in samples:
        print(f"   - BNS Sec {s[0]:8s} | Cog: {s[1] or 'N/A':15s} | Bail: {s[2] or 'N/A':15s} | Court: {s[3] or 'N/A'}")
    print("==================================================================")
    ...
    if with_embeddings:
        ...
        print("==================================================================")
        print(f" Total Rows with Embeddings in DB:             {embed_stats['total_embedded']} / {total_count}")
        print(f" Embedding Wall-Clock Time:                     {embed_stats['wall_clock_time']:.2f} seconds")
        print(f" Throughput:                                    {embed_stats['throughput']:.1f} rows/sec")
        print(f" Truncation Warnings (> 512 tokens):            {embed_stats['truncated_count']}")
        print("==================================================================\n")
    else:
...
def main():
    parser = argparse.ArgumentParser(description="Ingest First Schedule BNS offence classifications into Nyaya.")
    parser.add_argument(
        "--with-embeddings",
        action="store_true",
