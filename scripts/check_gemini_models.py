...
def discover_gemini_models():
    ...
    if not api_key or api_key in (
        ...
        print(
            "Please set a real Google Gemini API key in your .env file or GEMINI_API_KEY environment variable."
        )
    ...
    print(
        f"[*] API Key detected: {api_key[:6]}...{api_key[-4:] if len(api_key) > 10 else ''}"
    )
    print("[*] Configuring google.generativeai SDK...")
    genai.configure(api_key=api_key)

    print(
        "[*] Querying live Gemini API for supported models (genai.list_models())...\n"
    )
    try:
    ...
    if not target_model_name:
        target_model_name = (
            flash_models[0].name.replace("models/", "")
            if flash_models
            else content_models[0].name.replace("models/", "")
        )
    ...
    try:
        ...
        response = model.generate_content(
            "Respond with exactly: 'Gemini live connection verified.'"
        )
